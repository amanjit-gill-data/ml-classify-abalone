...
shucked.pred <- predict_wts(mlr.sh, abalone.t[,predictors])
shucked.actual <- abalone.t[,"Shucked.weight"]

plot(shucked.actual, shucked.pred); abline(a=0,b=1)
```
now viscera.
```{r}
viscera.pred <- predict_wts(mlr.vi, abalone.t[,predictors])
viscera.actual <- abalone.t[,"Viscera.weight"]

plot(viscera.actual, viscera.pred); abline(a=0,b=1)
```
