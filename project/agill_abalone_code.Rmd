...
est_value <- function(len, diam, height, v.sh, v.vi, alpha=0.05) {
  ...
  sh.pred <- PI(x0, abalone.t$Shucked.weight, mlr.sh, alpha)**2
  vi.pred <- PI(x0, abalone.t$Viscera.weight, mlr.vi, alpha)**2
  
  val <- calc_price(sh.pred$yhat, vi.pred$yhat, v.sh, v.vi)
  low.val <- calc_price(sh.pred$low, vi.pred$low, v.sh, v.vi)
  high.val <- calc_price(sh.pred$high, vi.pred$high, v.sh, v.vi)
