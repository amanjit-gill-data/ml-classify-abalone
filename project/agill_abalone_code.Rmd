...
```{r}
kable(summary(abalone[cols])) %>% kable_styling(full_width=TRUE, font_size = 11)
```
...
kable(outliers) %>% kable_styling(position="center", full_width=FALSE)
```
...
```{r echo=FALSE}
kable(mvn_test$multivariateNormality %>% select(-Statistic)) %>% kable_styling(full_width = FALSE, position = "float_left")

kable(mvn_test$univariateNormality %>% select(Variable, "p value", Normality)) %>% kable_styling(full_width = FALSE, position="right")
...
kable(out_compare) %>% kable_styling(full_width=FALSE)
```
...
kable(sk.final) %>% kable_styling(full_width=FALSE)
```
...
```
Highly significant, meaning the covariances are *not* equal. LDA can still be used, but we will start with QDA (Quadratic Discriminant Analysis), which does not depend on equal variances.
...
First define auxilliary functions.
```{r}
