...
mlr <- function(S, mu) {
  ...
  return(list(b, coeffs))
}

vars <- abalone.t %>% select(
  Shucked.weight, Viscera.weight, Length, Diameter, Height)

S.shucked <- cov(vars %>% select(-Viscera.weight))
mu.shucked <- colMeans(vars %>% select(-Viscera.weight))
mlr(S.shucked, mu.shucked)

S.viscera <- cov(vars %>% select(-Shucked.weight))
mu.viscera <- colMeans(vars %>% select(-Shucked.weight))
mlr(S.viscera, mu.viscera)
