...
1 - svm.rad$best.performance

```

RESULT: About the same as LDA.

compute accuracy of radial SVM for each class.
```{r}

cm.svm <- table(truth=abalone.t$Sex, prediction=fitted(svm.rad$best.model))

accuracy.svm.I <- cm.svm["I","I"]/sum(cm.svm["I",])
accuracy.svm.F <- cm.svm["F","F"]/sum(cm.svm["F",])
accuracy.svm.M <- cm.svm["M","M"]/sum(cm.svm["M",])
```

Terrible accuracy for females - 7%. 
...
I.notI <- I.notI %>% mutate(Sex=factor(Sex, levels=c("I", "NotI")))

cm.I.notI <- table(
    truth=I.notI$Sex, 
    prediction=qda(Sex ~ Length+Diameter+Height, data=I.notI, CV=TRUE)$class
)

sum(diag(cm.I.notI))/sum(cm.I.notI)
...
cm.I.notI <- table(
    truth=I.notI$Sex, 
    prediction=lda(Sex ~ Length+Diameter+Height, data=I.notI, CV=TRUE)$class
)
...
res <- rbind(table(lowR$Sex), table(highR$Sex))
row.names(res) <- c("Rings<=threshold", "Rings>threshold")

res <- cbind(res, proportionI=res[,1]/rowSums(res))
res
```
